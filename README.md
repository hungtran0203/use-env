[![Travis](https://travis-ci.org/toddbluhm/use-env.svg?branch=master)](https://travis-ci.org/toddbluhm/use-env)
[![Coverage Status](https://coveralls.io/repos/github/toddbluhm/use-env/badge.svg?branch=master)](https://coveralls.io/github/toddbluhm/use-env?branch=master)
[![npm](https://img.shields.io/npm/v/use-env.svg?maxAge=86400)](https://www.npmjs.com/package/use-env)
[![npm](https://img.shields.io/npm/dm/use-env.svg?maxAge=86400)](https://www.npmjs.com/package/use-env)
[![npm](https://img.shields.io/npm/l/use-env.svg?maxAge=2592000)](https://www.npmjs.com/package/use-env)
...
[![Greenkeeper badge](https://badges.greenkeeper.io/toddbluhm/use-env.svg)](https://greenkeeper.io/)
...
`npm install use-env` or `npm install -g use-env`
...
```sh
./node_modules/.bin/use-env node index.js
```
...
```sh
./node_modules/.bin/use-env -f ./custom/path/.env node index.js
```
...
Options:
  ...
  -r, --rc-file [path]                Custom rc file path (default path: ./.use-envrc(|.js|.json)
...
For more complex projects, a `.use-envrc` file can be defined in the root directory and supports
as many environments as you want. Simply use the `-e` flag and provide which environments you wish to
use from the `.use-envrc` file. Using multiple environment names will merge the environment variables
...
```sh
./node_modules/.bin/use-env -e production node index.js
# Or for multiple environments (where `production` vars override `test` vars,
# but both are included)
./node_modules/.bin/use-env -e test,production node index.js
```
...
```sh
./node_modules/.bin/use-env -f ./test/.env --use-shell "npm run lint && npm test"
```
...
```sh
./node_modules/.bin/use-env -f ./async-file.js node index.js
```

## Examples

You can find examples of how to use the various options above by visiting
the examples repo [use-env-examples](https://github.com/toddbluhm/use-env-examples).
...
- `.use-envrc` as valid json or `.use-envrc.json` in execution directory with at least one environment `{ "dev": { "key1": "val1" } }`
- `.use-envrc.js` JavaScript file exporting an `object` or a `Promise` that resolves to an `object` that contains at least one environment
...
- **`options`** { `object` }
  ...
  - **`rc`** { `object` }
    ...
    - **`filePath`** { `string` }: Custom path to the `.rc` file (defaults to: `./.use-envrc(|.js|.json)`)
  - **`options`** { `object` }
